...
export const updateAcceptanceStatus = async (req, res, next) => {
  try {
    ...
    // Check if both user and maid profiles are active
    if (!maidHire.user || maidHire.user.profile_status !== 'ACTIVE') {
      return res.status(403).json({message: 'User profile is not active. Cannot update acceptance status.'});
    }
    if (!maidHire.maid || maidHire.maid.profile_status !== 'ACTIVE') {
      return res.status(403).json({message: 'Maid profile is not active. Cannot update acceptance status.'});
    }
...
export const createStripeCheckoutSession = async (req, res, next) => {
  try {
    ...
    if (!maidHire || !maidHire.user || !maidHire.maid) {
      return res.status(404).json({message: 'Maid hire record not found'});
    }
    if (maidHire.user.profile_status !== 'ACTIVE') {
      return res.status(403).json({message: 'User profile is not active. Cannot create payment session.'});
    }
    if (maidHire.maid.profile_status !== 'ACTIVE') {
      return res.status(403).json({message: 'Maid profile is not active. Cannot create payment session.'});
    }
